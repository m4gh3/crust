...
struct circ_buf_t
{
	...
	{
		return s+1+i >= sz ? buf[s+1+i-sz] : buf[s+1+i];
	}
	size_t size()
	{ return sz_; }
};
...
void match(match_shared_t &m, rrex_tree *next, int reduce, int len=0, int idx=0 )
{
		...
		for( auto it = next->lower_bound({c,c}); it != next->end(); it++ )
		{
			if( it->first.a <= c && c <= it->first.b )
			{
				if( it->second.next != nullptr )
					match(m, it->second.next, -1, 0, idx+1 );
				if( it->second.reduce >= 0 )
				{
					...
					match(m, m.root, it->second.reduce, 0, idx );
				}
				to_check--;
			}
		}
